...
const App = createApp({
  ...
  onShow (options) {
    // 判断是否从第三方小程序跳转
    const extraData = options.referrerInfo?.extraData // 兼容支付宝options默认没有referrerInfo字段
    if (extraData) {
      Taro.setStorageSync('certToken', extraData.certToken)
      Taro.setStorageSync('loginType', 1) // 0.小程序内部运行，1.第三方小程序跳转，2.h5跳转
    }
    // 判断是否从外部h5跳转
    const certToken = options.query?.certToken
    if (certToken) {
