<template>
  <view class="container">
    <block v-if="!authList.length">
      ...
      <view class="no-auth-record-tips"
        >暂无{{ ['认证记录', '认证请求'][flag] }}</view
      >
    </block>
    <block v-else>
        ...
        >
          <view class="item-content">
            ...
            <template v-if="flag">
              ...
              <view
                class="auth-info flex"
                v-if="item.hour || item.minute || item.second"
                >还剩： <view class="emphasize">{{ item.hour }}</view
                >小时 <view class="emphasize">{{ item.minute }}</view
                >分 <view class="emphasize">{{ item.second }}</view
                >秒
              </view>
            </template>
            <template v-else>
          ...
          <nut-button
            v-if="flag"
            shape="square"
            :color="item.isExpired ? '#ddd' : '#0a7aee'"
            @tap.stop="toAuth(item)"
            >马上认证</nut-button
          >
          <view class="auth-result" v-else>
            <view :class="['auth-status', authResult[item.authResStr].class]">{{
              authResult[item.authResStr].text
            }}</view>
            <nut-icon
              name="right"
              class="arrow"
...
const calRestTime = target => {
  ...
  for (let i = 0; i < target.length; i++) {
    ...
    const expireTime = item.expireTime.replace(/-/g, '/') // 兼容ios日期识别的bug
    const expireTimestamp = Date.parse(expireTime)
    const restTimestamp = expireTimestamp - timestamp
    if (restTimestamp > 0) {
      const hour = parseInt(restTimestamp / (60 * 60 * 1000))
      const deduct_hour = restTimestamp - hour * (60 * 60 * 1000)
      const minute = parseInt(deduct_hour / (1000 * 60))
      const deduct_minute = deduct_hour - minute * (1000 * 60)
      const second = parseInt(deduct_minute / 1000)
